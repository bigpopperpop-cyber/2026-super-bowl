      ...
      <div className="p-2 bg-slate-900 border-b border-slate-800 flex justify-between items-center shrink-0">
        ...
        <div className="flex -space-x-2">
          {users.slice(0, 5).map(u => (
            <div key={u.id} className="w-6 h-6 rounded-full border-2 border-slate-900 bg-slate-800 overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-slate-700 opacity-50">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">Waiting for Mesh Signal...</p>
          </div>
        )}
            ...
            <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
              ...
              <div className={`flex items-end gap-2 max-w-[90%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`px-4 py-2.5 rounded-2xl text-sm border shadow-xl ${
                  isMe ? 'bg-blue-600 border-blue-500 text-white rounded-tr-none' : 'bg-slate-800 border-slate-700 text-slate-200 rounded-tl-none'
                }`}>
                  {msg.text}
                </div>
                {isMe && <i className="fas fa-check-double text-[8px] text-blue-400 mb-1 opacity-50"></i>}
              </div>
            </div>
          );
        })}
      ...
      <div className="shrink-0 bg-slate-900 border-t border-slate-800">
        {showEmojiPicker && (
          <div className="p-3 border-b border-slate-800 grid grid-cols-8 gap-2 bg-slate-900/90 backdrop-blur-md">
            {COMMON_EMOJIS.map(e => <button key={e} onClick={() => setInputText(p => p + e)} className="text-lg p-2 hover:bg-white/10 rounded-xl transition-all active:scale-125">{e}</button>)}
          </div>
        )}
